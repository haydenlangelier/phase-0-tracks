...
#require relative means the file needs a method/class from seperate file that is located
# on your pc not in the cloud


class VirusPredictor
#runs automatically when the method is called and includes several attributes
  def initialize(state_of_origin, population_density, population)
    @state = state_of_origin
    @population = population
    @population_density = population_density
  end
#user facing output which takes both mini algorithms and runs them with their key parameters
...
#function that calculates death given three key parameters
...
#function that calculates rate of spread given three parameters
...
STATE_DATA.each_key do |state| 
  all50states = VirusPredictor.new(state, STATE_DATA[state][:population_density], STATE_DATA[state][:population])
all50states.virus_effects
end
